...
void check_function(struct node *function) {
    ...
    check_parameters(getchild(function, 1)); // passa o nó parameters para dar check aos parametros
...
void check_parameters(struct node *parameters){
    if (parameters->category != Parameters)
    {
        printf("Parameters must be a Parameters node\n");
        semantic_errors++;
    }
    
    struct node_list *childs = parameters->children;
    ...
    */
    while((childs = childs->next) != NULL){
        if (search_symbol(symbol_table, childs->node->children->next->next->node->token) == NULL)
        {
            insert_symbol(symbol_table, childs->node->children->next->next->node->token, no_type, parameters);
        } else {
            printf("Identifier %s already declared\n", childs->node->children->next->next->node->token);
            semantic_errors++;
        }
    }
}
